...
def greedy_cow_transport(cows,limit=10):
    ...
    while sum(cowsCopy.values()) > 0:
        ...
        for name, weight in cowsSorted:
            if cowsCopy[name] != 0 and weight + total <= limit:
                trip.append(name)
                total += weight
                cowsCopy[name] = 0
        result.append(trip)
...
def brute_force_cow_transport(cows,limit=10):
    ...
    for partition in get_partitions(cows.keys()):
        powerSet.append(partition)
    powerSet.sort(key=len)

    validTrips = powerSet.copy()

    for partition in powerSet:
        for trip in partition:
            total = sum([cows.get(cow) for cow in trip])
            if total > limit:
                validTrips.remove(partition)
                break

    return min(validTrips, key=len)
...
def compare_cow_transport_algorithms():
    ...
    brute_results = brute_force_cow_transport(cows, limit=10)
    brute_end = time.time()
    ...
    print('Brute Force Algorithm results:', brute_results)
    print('Number of trips returned by Brute Force Algorithm:', len(brute_results))
